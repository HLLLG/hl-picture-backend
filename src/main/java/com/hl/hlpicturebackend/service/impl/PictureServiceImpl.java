...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest,
        ...
        transactionTemplate.execute(status -> {
            ...
            boolean update = spaceService.lambdaUpdate()
                    .eq(Space::getId, finalSpaceId)
                    .setSql("totalSize = totalSize + " + picture.getPicSize() + "-" + finalOldPicture.getPicSize() == null
                            ? "0" : finalOldPicture.getPicSize().toString())
                    .setSql("totalCount = totalCount + 1" + "-" + (finalOldPicture == null ? "0" : "1"))
                    .update();
            ThrowUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "空间额度更新失败");
            return picture;
        });
